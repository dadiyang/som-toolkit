...
学员操作失败后给出的归因可能完全错误。例如：
- 学员说"搜索结果是 WebView 读不到" → 实际是 deeplink 没执行成功，它在错误的页面上
- 教练直接把学员的结论写进技能文件 → 教材被污染

### 写入技能文件前必须独立验证

任何关于平台特性（渲染方式、WebView 判断、App 行为）的结论，教练必须：
1. 自己跑一次相同操作，确认结果
2. 用客观方法验证（如 `class_name` 检查 WebView，而非"元素少"推断）
3. 不接受"看起来合理"的推测作为结论

### 修正结论时也要验证

发现之前结论可能错误时，不要急于用另一个推测替代。先拿数据。
